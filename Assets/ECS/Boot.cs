...
public class Boot : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        var enemyPlanes = UnitEnityDefinitions.SetupAIPlanes(UnitEnityDefinitions.enemyPlaneArechetype, 20, gd.EnemyPlaneMesh, gd.enemyPlaneMaterial);

        for (int i = 0; i < enemyPlanes.Length; i++)
        {
            entityManager.SetComponentData(enemyPlanes[i], new TargetSelection { target = player });
            //entityManager.SetComponentData(enemyPlanes[i], new RotationSpeed {Value = 0.1f});
