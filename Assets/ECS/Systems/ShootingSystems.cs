...
public class ShootingSystem : ComponentSystem
{
    ...
    protected override void OnUpdate()
    {
        ...
        Entities.WithAll<UseUnscaledDeltatime>().ForEach((ref Translation translation, ref Rotation rotation, ref Velocity velocity, ref LocalToWorld localToWorld, ref ShootWeapon shootWeapon) =>
        {
            if (shootWeapon.shoot && shootWeapon.cooldownTimer <= 0)
            {
                var spawnPos = translation.Value + localToWorld.Up * -1 + localToWorld.Forward * 1.1f + localToWorld.Right * shotXOffset;
            ...
            {
                shootWeapon.cooldownTimer -= Time.unscaledDeltaTime;
            }
        });
    }
}
